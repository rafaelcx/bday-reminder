...
class LogMiddleware implements MiddlewareInterface {
    ...
    private function logIncomingRequestDetails(Request $request): void {
        ProcessLogContext::set('process_type', 'http');
    ...
    private function flushProcessLogs(Response $response): void {
        ...
        ProcessLogContext::set('http_response_status', $response_status);

        if ($response_status == '302') {
            ProcessLogContext::set('http_response_redirect_location', $response->getHeaderLine('Location'));
        }
